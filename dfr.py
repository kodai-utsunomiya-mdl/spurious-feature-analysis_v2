...
# #     # # 指標の計算
# #     # print(" [Analysis] Computing Geometric Metrics on Balanced Test Set...")
# #     # geo_metrics = compute_geometric_metrics(X_test_bal_scaled, y_test_bal, a_test_bal)
# #     # # -------------
...
#         # Standard Scalerのパラメータ (mean, scale)
#         # --- [Experiment] 標準化を無効化するためダミー(0, 1)を設定 ---
#         # self.scaler_mean = torch.from_numpy(scaler.mean_).float()
#         # self.scaler_scale = torch.from_numpy(scaler.scale_).float()
#         self.scaler_mean = torch.zeros(input_dim).float()
#         self.scaler_scale = torch.ones(input_dim).float()
...
#     # StandardScaler は Trainデータ で Fit させる (原論文の実装に準拠)
#     # --- [Experiment] 標準化を無効化する (Fitしない) ---
#     print("Fitting StandardScaler on TRAIN data... (SKIPPED for experiment)")
#     scaler = StandardScaler()
#     # scaler.fit(train_embeddings)
    
#     # 後続のコードで scaler.mean_, scaler.scale_ を参照するため，
...
#     # Check for 0/1 and convert to -1/1 for moment calculations
#     y_moment = y_vec.copy()
#     if np.all(np.isin(np.unique(y_moment), [0, 1])):
#         y_moment = 2 * y_moment - 1
        
#     a_moment = a_vec.copy()
#     if np.all(np.isin(np.unique(a_moment), [0, 1])):
#         a_moment = 2 * a_moment - 1
...
#     # --- Moment Calculations for Validity Check ---
#     # 1. E[YA h]
#     ya_moment = y_moment * a_moment
#     E_YA_h = np.mean(ya_moment[:, None] * Z, axis=0)
#     metrics['norm_E_YA_h'] = np.linalg.norm(E_YA_h)
    
#     # 2. E[YA hh^T]
#     # Matrix size m x m.
#     Z_ya = Z * ya_moment[:, None]
#     E_YA_hhT = (Z_ya.T @ Z) / N
#     metrics['norm_E_YA_hhT'] = np.linalg.norm(E_YA_hhT)
    
#     # 3. E[A hh^T]
#     Z_a = Z * a_moment[:, None]
#     E_A_hhT = (Z_a.T @ Z) / N
#     metrics['norm_E_A_hhT'] = np.linalg.norm(E_A_hhT)
...
#     # Added for Validity Check
#     print(f"    Moment Validity Check (Expect ~0):")
#     print(f"      || E[YA h] ||: {geo_metrics.get('norm_E_YA_h', 0.0):.6f}")
#     print(f"      || E[YA hh^T] ||: {geo_metrics.get('norm_E_YA_hhT', 0.0):.6f}")
#     print(f"      || E[A hh^T] ||: {geo_metrics.get('norm_E_A_hhT', 0.0):.6f}")
